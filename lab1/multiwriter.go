...
func any_errors(errors []error) bool {
...
func WriteTo(b []byte, writers ...io.Writer) (n []int, errs Errors) {
	...
	for _, w := range writers {
		a, e := w.Write(b)
		if a < len(b) {
			myerrors = append(myerrors, io.ErrShortWrite)
		} else {
			myerrors = append(myerrors, e)
		}
		myints = append(myints, a)
	}

	if any_errors(myerrors) {
		return myints, myerrors
	} else {
		return myints, nil
	}
}
